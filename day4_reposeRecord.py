...
def partAandB(logs):
    ...
    sleepiest = max(timeSlept, key = timeSlept.get)

    print("Part A: %d" % (sleepiest * max(range(60), key = lambda minute: sum(start <= minute < end for start, end in guardLogs[sleepiest]))))

    frequency, minute, guard = max((sum(start <= minute < end for start, end in guardLogs[guard]), minute, guard) for minute in range(60) for guard in guardLogs)
    print("Part B: %d" % (minute * guard))
